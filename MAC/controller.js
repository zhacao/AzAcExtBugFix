  ...
  vscode.window.showInputBox({
    ...
    .then(runbookName => {
      if (runbookName != undefined) {
        ...
        vscode.window.showQuickPick(["PowerShell", "PowerShell Workflow", "Python"])
          .then(runbookType => {
            var runtimeVersions = [];
            switch (runbookType) {
              case 'PowerShell':
            ...
            vscode.window.showQuickPick(runtimeVersions, {
              ...
              .then(runbookRuntime => {
                Azure.doesRunbookExist(runbookName, function (runbookExist) {
                  if (!runbookExist) {
                    Azure.createLocalRunbook(runbookName, runbookType.replace(' ', ''), undefined, undefined, function () {
                      Azure.createAzureRunbook(runbookType, runbookRuntime, function () {
